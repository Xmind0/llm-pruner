...
class CompressedLlamaDecoderLayer(LlamaDecoderLayer):
    ...
    def forward(
        ...
        **kwargs,
        ...
        self_attn_weights = outputs[1] if len(outputs) > 1 else None
...
class LlamaModelAdapter(ModelAdapter):
    ...
    def _from_pretrained(
        ...
        *,
        ...
        if not (model_name.startswith("meta-llama/Llama-2") or model_name.startswith("meta-llama/Meta-Llama-3") or model_name == "JackFram/llama-68m" or "llama-2" in model_name.lower()):
            return None
